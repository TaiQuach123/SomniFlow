...
from src.agents.harm.nodes import task_handler_node, retriever
from langgraph.graph import START, END, StateGraph


def create_harm_subgraph():
    harm_subgraph_builder = StateGraph(HarmState, output=HarmOutputState)
    harm_subgraph_builder.add_node("task_handler_node", task_handler_node)
    harm_subgraph_builder.add_node("retriever", retriever)

    harm_subgraph_builder.add_edge(START, "task_handler_node")
    harm_subgraph_builder.add_edge("task_handler_node", "retriever")
    harm_subgraph_builder.add_edge("retriever", END)
